...
namespace vkengine {
    void VKPipeLineHandle::createDeferredPBRPipeline(VkFormat outColorFormat, VkFormat depthFormat, VkSampleCountFlagBits msaaSamples)
    {
        this->name = "pbrdeferred";
        ...
        std::vector<VkPipelineShaderStageCreateInfo> shaderStagesCI = shaderManager.createPipelineShaderStageCIs(name);

        std::vector<VkFormat> outColorFormats{ outColorFormat };
        ...
        vertexInputBindingDesc[0].stride = sizeof(Vertex2);
