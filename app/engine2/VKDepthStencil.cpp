...
namespace vkengine
{
    VKDepthStencil::VKDepthStencil(VKcontext &ctx) : ctx(ctx)
    {
    }

    void VKDepthStencil::create(cUint32_t width, cUint32_t height, VkSampleCountFlagBits msaaSamples)
    {
        ...
        helper::resource::createImageView(
            device,
            image,
            depthFormat,
            VK_IMAGE_ASPECT_DEPTH_BIT | ((depthFormat >= VK_FORMAT_D16_UNORM_S8_UINT) ? VK_IMAGE_ASPECT_STENCIL_BIT : 0),
            1, // mipLevels
            1  // imageCount
        );
        ...
        helper::resource::createImageView(
            ...
            image,
            depthFormat,
            VK_IMAGE_ASPECT_DEPTH_BIT, // Only depth aspect for sampling
            1, // mipLevels
            1  // imageCount
        );

        this->resourceBinding.image = this->image;
        this->resourceBinding.imageView = this->samplerView; // Use depth-only view for sampling
        this->resourceBinding.descriptorCount = 1;
        this->resourceBinding.update();
        this->resourceBinding.getBarrierHelper().update(depthFormat, 1, 1);
        #endif
    }
