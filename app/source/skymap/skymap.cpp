...
namespace vkengine {
    ...
    void SkyCubeEngine::update(float dt)
    {
    ...
    void SkyCubeEngine::recordCommandBuffer(FrameData* framedata, uint32_t imageIndex)
    {
        // 커맨드 버퍼 기록을 시작합니다.
    ...
    {
        this->cubeObject->createIndexBuffer(const_cast<std::vector<uint32_t>&>(cubeindices_));
        this->cubeSkybox->createIndexBuffer(const_cast<std::vector<uint32_t>&>(skyboxIndices));
    }
    ...
    void SkyCubeEngine::createDescriptorSets()
    {
    ...
    void SkyCubeEngine::cleanupSwapcChain()
    {
