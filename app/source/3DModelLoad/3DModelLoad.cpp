...
namespace vkengine {
    ...
    Load3DModelEngine::Load3DModelEngine(std::string root_path) : VulkanEngine(root_path) {
        this->cubeSkybox = nullptr;
        this->modelObject = nullptr;
    }

    Load3DModelEngine::~Load3DModelEngine() {}

    void Load3DModelEngine::init()
    {
        VulkanEngine::init();
        this->camera = std::make_shared<vkengine::object::Camera>();
    }

    bool Load3DModelEngine::prepare()
    {
    ...
    void Load3DModelEngine::cleanup()
    {
    ...
    void Load3DModelEngine::drawFrame()
    {
        ...
        this->updateUniformBuffer2(static_cast<uint32_t>(this->currentFrame), this->modelObject->getMatrix());
    ...
    bool Load3DModelEngine::init_sync_structures()
    {
    ...
    void Load3DModelEngine::recordCommandBuffer(FrameData* framedata, uint32_t imageIndex)
    {
        // 커맨드 버퍼 기록을 시작합니다.
    ...
    void Load3DModelEngine::createVertexbuffer()
    {
    ...
    void Load3DModelEngine::createDescriptorSetLayout()
    {
    ...
    void Load3DModelEngine::createGraphicsPipeline()
    {
        ...
        VkShaderModule baseFragShaderModule = this->VKdevice->createShaderModule(this->RootPath + "../../../../../../shader/fragloadmodel.spv");
    ...
    void Load3DModelEngine::createGraphicsPipeline2()
    {
    ...
    void Load3DModelEngine::updateUniformBuffer(uint32_t currentImage)
    {
    ...
    void Load3DModelEngine::updateUniformBuffer2(uint32_t currentImage, glm::mat4 matrix)
    {
        static auto startTime = std::chrono::high_resolution_clock::now();
        auto currentTime = std::chrono::high_resolution_clock::now();
        float time = std::chrono::duration<float, std::chrono::seconds::period>(currentTime - startTime).count();
        ...
        UniformBufferObject ubo{};
        ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f)) * matrix;
        ubo.view = this->camera->getViewMatrix();
        ubo.proj = this->camera->getProjectionMatrix();
    ...
    void Load3DModelEngine::cleanupSwapcChain()
    {
