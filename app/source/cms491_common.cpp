...
namespace vkutil {
    namespace helper
    {
        ...
        void copyBuffer(VkDevice& VKdevice, VkCommandPool& VKcommandPool, VkQueue& graphicsVKQueue, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
        {
            ...
            vkBeginCommandBuffer(commandBuffer, &beginInfo); // 복사 명령을 기록합니다.

            VkBufferCopy copyRegion{};
            copyRegion.srcOffset = 0; // Optional
            copyRegion.dstOffset = 0; // Optional
            copyRegion.size = size;
            vkCmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, 1, &copyRegion); // 복사 명령을 기록합니다.
            
            vkEndCommandBuffer(commandBuffer); // 복사 명령을 종료합니다.
