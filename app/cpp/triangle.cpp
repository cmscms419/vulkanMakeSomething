...
namespace vkengine {
    ...
    void triangle::recordCommandBuffer(FrameData* framedata, uint32_t imageIndex)
    {
        ...
        vkCmdBeginRenderPass(framedata->mainCommandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            // 디스크립터 세트를 바인딩합니다.
            vkCmdBindDescriptorSets(framedata->mainCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, this->VKpipelineLayout, 0, 1, &this->VKdescriptorSets[currentFrame], 0, nullptr);
            // 렌더 패스를 종료합니다.
    ...
    {
        VkDeviceSize bufferSize = sizeof(vkengine::object::UniformBufferObject);
